...
class Trainer(object):
    ...
    def load_data(self):
        ...
        self.args.num_node_features = dataset.data.num_node_features
        if self.args.num_node_features == 0:
            print('Added Node Feature')
            self.args.num_node_features = dataset.num_features
        
        self.args.num_edge_features = dataset.data.num_edge_features
        if self.args.num_edge_features == 0:
            print('Added Edge Feature')
            self.args.num_edge_features = 1

        self.args.num_classes = dataset.num_classes
        self.args.avg_num_nodes, self.args.avg_num_edges = np.ceil(np.mean([data.num_nodes for data in dataset])), np.ceil(np.mean([data.num_edges for data in dataset]))

        print('# %s: [NODE FEATURES]-%d [EDGE FEATURES]-%d [NUM_CLASSES]-%d [AVG_NODES]-%d [AVG_EDGES]-%d' % (dataset, self.args.num_node_features, 
                self.args.num_edge_features, self.args.num_classes, self.args.avg_num_nodes, self.args.avg_num_edges))

        print(f"Nodes : {np.mean([data.num_nodes for data in dataset]):.2f}      Edges : {np.mean([data.num_edges for data in dataset])/2:.2f}")
    ...
    def load_model(self):
        ...
        if self.args.model == 'HyperDrop':

            model = Model_HyperDrop(self.args)
